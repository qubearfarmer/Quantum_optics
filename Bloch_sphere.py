...
# bSphere.add_annotation((up+np.exp(1j*np.pi/2)*down)/np.sqrt(2.2), text=r"$\frac{|0\rangle + e ^{i\pi/2}|1\rangle}{\sqrt{2}}$")
# #
# sup_state4 = (up-np.exp(1j*np.pi/2)*down)/np.sqrt(2)
# bSphere.add_states(sup_state4)
# bSphere.add_annotation((up-np.exp(1j*np.pi/2)*down)/np.sqrt(2), text=r"$\frac{|0\rangle - e ^{i\pi/2}|1\rangle}{\sqrt{2}}$")
#
